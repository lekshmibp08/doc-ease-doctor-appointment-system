...
export const initializeSocket = (server: any) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send message", (message) => {
      ...
      console.log(
        `Message sent in chat: ${chatId} from ${senderId} to ${receiverId}: ${text}`
      );
    });
    ...
    socket.on("start video call", ({ chatId, receiverId, callerId }) => {
      console.log(
        `Starting video call from ${callerId} to ${receiverId} in chat ${chatId}`
      );
    ...
    socket.on("accept call", ({ chatId, callerId, receiverId }) => {
      ...
      console.log(
        `Call accepted in chat ${chatId}, Caller: ${callerId} and Receiver: ${receiverId}`
      );
    });
    ...
    socket.on("end call", ({ chatId, userId, receiverId }) => {
      console.log(
        `Call ending in chat ${chatId} between ${userId} and ${receiverId}`
      );
      ...
      console.log(
        `Call ended in chat ${chatId} between ${userId} and ${receiverId}`
      );
    });
