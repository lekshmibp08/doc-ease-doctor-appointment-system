...
const UserChat: React.FC<UserChatProps> = ({ isOpen, onClose, initialDoctorId }) => {
  ...
  useEffect(() => {
    ...
    const fetchChats = async () => {
      try {
        const response = await axios.get("/api/users/get-chats", {
          params: { userId },
        })
  ...
  const createNewChat = async (doctorId: string) => {
    try {
      const response = await axios.post("/api/users/chat", { doctorId, userId })
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post("/api/users/send-message", messageData)
